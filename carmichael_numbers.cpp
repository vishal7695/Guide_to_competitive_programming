...
A Carmichael number is an odd composite number n which satisfies Fermat's little theorem
    a^(n-1) = 1 (mod n) 	for every choice of a satisfying (a,n)=1 (i.e., a and n are relatively prime)
with 1<a<n.
...
int main()
{
    ...
    {
        cout<<endl;
        Fermat(n,5)?cout<<"prime\n":cout<<"composite";
    }

    // probability of wrong results is less, but it is there
