...
int lazyRangeMinQuery(vector<int> &segment,vector<int> &lazy, int l, int r, int low, int high, int root)
{
    if(l>high || r<low)
    {
        //no overlap
    ...
    //partial overlap
    int mid=(low+high)/2;
    return min(lazyRangeMinQuery(segment, lazy,l, r, low, mid, 2*root+1), lazyRangeMinQuery(segment, lazy, l, r, mid+1, high, 2*root+2));
}

//update the value of elements in the range to be value
void lazyUpdateQuery(vector<int> &segment, vector<int> &lazy, int l, int r, int value, int low, int high, int root)
{
    if(lazy[root]!=0)
    {
        //this node needs to be updated
...
int main()
{
    ...
    for(int i=1;i<=m;i++)
    {
        ...
        for(j=1;j<=m2;j++)
        {
                //range for the query
                cin>>l>>r;
                cout<<l<<","<<r<<"- "<<lazyRangeMinQuery(segment,lazy,l,r,0,n-1,0)<<endl;
        }
        ...
        lazyUpdateQuery(segment,lazy,l,r,value,0,n-1,0);
    }
