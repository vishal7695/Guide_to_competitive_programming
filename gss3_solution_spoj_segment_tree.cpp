//solution for problem gss3 spoj using segment trees
...
void updateQuery(vector<st> &segment,int l,int r,int value,int low, int high, int root)
{
    //cout<<l<<" "<<low<<" "<<high<<" "<<root<<" "<<segment[root].sum<<" "<<segment[root].left<<" "<<segment[root].right<<" "<<segment[root].value;
    ...
    {
        //cout<<" updated"<<endl;
        segment[root].sum=segment[root].left=segment[root].right=segment[root].value=value;
        return ;
    }
    ...
    //cout<<"  "<<low<<" "<<high<<" "<<segment[root].sum<<" "<<segment[root].left<<" "<<segment[root].right<<" "<<segment[root].value<<endl;
}
...
/*
    ...
    for(j=0;j<=2*n-1;j++)
    {
        cout<<j<<" "<<segment[j].sum<<" "<<segment[j].left<<" "<<segment[j].right<<" "<<segment[j].value<<endl;
    }
...
*/
    ...
    W(m)
    {
        ...
        if(y==0)
        {
            ...
            updateQuery(segment,x-1,x-1,y,0,n-1,0);
/*
            cout<<endl<<"display segment tree"<<endl;
            for(j=0;j<=2*n-1;j++)
            {
